...
### Block 类型枚举
参考 packages/amplication-server/src/enums/EnumBlockType.ts
...
项目配置通过 `ProjectConfigurationSettings` Block 存储，其 DTO 定义参考：packages/amplication-server/src/core/projectConfigurationSettings/dto/ProjectConfigurationSettings.ts
...
源码：`packages/amplication-server/src/core/resource/serviceTemplate.service.ts`（`upgradeServiceToLatestTemplateVersion` 函数）
...
```
前置校验 → 对比版本差异 → 合并 Block（等待全部完成）
    ↓
更新模板版本记录 → 标记告警为已解决 → 返回
```

**实际执行顺序（存在两个 Bug 导致提前更新）**：

由于 `forEach` 返回值和 `Promise.all` 嵌套数组两个问题，合并步骤完全不被等待，导致模板版本记录和告警状态在 Block 合并完成前就已更新。

#### Bug 1：`forEach` 返回 `undefined`

```typescript
// ❌ Array.prototype.forEach() 不返回任何值，始终返回 undefined
// updatedBlocks 中的所有更新操作（含 CodeEngineVersion 同步）都启动但不被等待
...
// Promise.all 不会递归展开嵌套数组：数组对象本身不是 Promise，会被当作普通值立即 resolve
await Promise.all([
  changes.createdBlocks.map(...),   // Promise[] — 不被等待
  changes.deletedBlocks.map(...),   // Promise[] — 不被等待
  changes.updatedBlocks.forEach(...), // undefined — 立即 resolve
]);
```

#### 模板版本记录与告警状态提前更新的统一原因

两个 Bug 共同作用，使得 `Promise.all` 调用**立即 resolve**，后续步骤在 Block 合并完成前就开始执行：

```
时间轴 ──────────────────────────────────────────────────────────────────────►

T1 compareResourceVersions()         ✅ 版本差异计算完成
│
...
│
T4 updateResourceTemplateVersion()  ✅ 模板版本记录被更新为新版本 ← 提前更新
   └─ packages/amplication-server/src/core/resourceTemplateVersion/resourceTemplateVersion.service.ts
│
T5 resolvesServiceTemplateUpdated() ✅ 告警被批量标记为 Resolved    ← 提前更新
   └─ packages/amplication-server/src/core/outdatedVersionAlert/outdatedVersionAlert.service.ts
│
T6 HTTP 响应返回 → 前端认为升级已完成
│
   ... 后台异步任务仍在执行（可能成功也可能异常失败）...
│
T7/T8 插件/代码引擎版本同步这时才可能完成（或因异常永远未完成）
```
...
`handleMergeDeletedBlock` 目前是空实现，模板中删除的插件不会同步到目标资源：

```typescript
async handleMergeDeletedBlock(...): Promise<void> {
  return;  // ⚠️ 空实现
}
```
...
```typescript
// ✅ 使用展开运算符扁平化 Promise 数组，用 map 替代 forEach
await Promise.all([
  ...changes.createdBlocks.map(bv =>
    this.handleMergeCreatedBlock(resourceId, bv, user, mergeOptions)
  ),
  ...changes.deletedBlocks.map(bv =>
    this.handleMergeDeletedBlock(resourceId, bv, user)
  ),
  ...changes.updatedBlocks.map(diff =>
...
由于 `Promise.all` 实际不等待任何后台任务（详见 3.5 节），Block 合并过程中抛出的异步异常完全不会被捕获：

- 插件安装/更新失败（如缺少认证实体、重复安装）不会中断流程，也不会被记录
- 代码引擎版本同步失败不会触发任何回滚或告警
- 模板版本记录和告警状态已提前更新，系统整体呈现"升级成功"的状态
- 实际结果：资源处于版本标记与真实配置不一致的状态

如果将来修复了 `Promise.all` 等待问题，则合并过程中任一异常会导致 `updateResourceTemplateVersion()` 和 `resolvesServiceTemplateUpdated()` 不执行，资源版本记录仍指向旧版本。
