...
Lookup 正向生成在 `@relation(...)` 参数上存在以下硬性限制（见 [create-prisma-schema-fields.ts#L331-L346](packages/data-service-generator/src/server/prisma/create-prisma-schema-fields.ts#L331-L346) 的 `createObjectField` 调用）：

| Prisma @relation 参数 | 正向生成支持情况 | 代码证据 |
|----------------------|-----------------|---------|
| `references` | ❌ **固定硬编码为 `["id"]`** | L338-L341 传入 `[scalarRelationFieldName]` + 硬编码 `["id"]`，源码注释明确标注 `@todo: calculate the referenced field on the related entity (currently it is always 'id')`，未读取关联实体的实际主键字段名 |
| `onDelete` | ❌ 不支持，传 `undefined` | L342 参数位置对应 onDelete，始终为 `undefined`；同时 [lookup.json](libs/util/code-gen-types/src/schemas/lookup.json) 中无 onDelete / onUpdate / relationMap 属性定义 |
| `onUpdate` | ❌ 不支持，传 `undefined` | L343 参数位置对应 onUpdate，始终为 `undefined` |
| `map` | ❌ 不支持，传 `undefined` | L344 参数位置对应 relation map，始终为 `undefined` |
| `name` | ✅ 部分支持（仅多关系时生成） | L337，见 3.2 节 `createRelationName()` 命名算法 |
| `fields` | ✅ 完整支持 | L338，使用 `field.properties.fkFieldName` |

因此正向生成的 `@relation` 输出形态固定为：

```prisma
// 单关系路径
customer Customer @relation(fields: [customerId], references: [id])

// 多关系路径（含 relationName）
manager Employee? @relation(name: "OrgChart", fields: [managerId], references: [id])
```

永远不会出现 `onDelete: Cascade`、`onUpdate: SetNull` 或 `map: "custom_column"` 等高级参数。
...
#### 6.2.6 反向导入的关系约束边界（@relation 整体过滤）

反向导入中，Lookup 字段的所有 `@relation(...)` 属性（包括其内部参数）会被整体过滤掉，**不会写入 customAttributes，也不会解析为独立字段属性**。

过滤逻辑位于 [helpers.ts](packages/amplication-server/src/core/prismaSchemaParser/helpers.ts) 的 `removeAutoGeneratedRelationAttribute()`（L69-L71）：
...
`filterOutAmplicationAttributesBasedOnFieldDataType()` 在 `EnumDataType.Lookup` 分支调用此函数（helpers.ts L91-L92）。由于判断条件为 `.startsWith("@relation")`，整个 `@relation(...)` 字符串（无论包含多少子参数）都会被一次性过滤，因此以下参数全部无法保留：

| Prisma @relation 参数 | 反向导入保留情况 | 原因 |
|----------------------|-----------------|------|
| `fields: [...]` | ❌ 丢失 | 被整体过滤，外键列信息另行通过 `isFkFieldOfARelation` 跳过独立字段创建，但不会还原为 `fkFieldName` |
| `references: [...]` | ❌ 丢失 | 被整体过滤，反向导入假设关联实体主键为 `id` |
| `name: "..."` | ❌ 丢失 | 被整体过滤，多关系路径的命名信息丢失，反向导入重新走 7 步管线的命名冲突处理 |
| `onDelete: Cascade/Restrict/SetNull/NoAction/SetDefault` | ❌ 丢失 | 被整体过滤，[lookup.json](libs/util/code-gen-types/src/schemas/lookup.json) 中无对应属性槽位，正向生成也不支持 |
| `onUpdate: Cascade/...` | ❌ 丢失 | 被整体过滤，同上 |
| `map: "..."` | ❌ 丢失 | 被整体过滤，无属性槽位 |
...
| Lookup 的 `@relation(fields: [...])` 外键列名 | ❌ 丢失 | 反向导入整体过滤 `@relation` 属性，外键列通过 `isFkFieldOfARelation` 判定并跳过独立字段创建，**不会**还原为 `fkFieldName` 属性 |
| Lookup 的 `@relation(references: [...])` 引用字段 | ❌ 丢失且不影响正向 | 反向导入整体过滤 `@relation`，**假设**引用字段为 `id`；即使原始 Prisma schema 引用了非 `id` 字段也不会保留；正向生成本身也**硬编码**输出 `references: [id]`（含 `@todo` 注释） |
| Lookup 的 `@relation(name: "...")` 多关系命名 | ❌ 丢失 | 反向导入整体过滤 `@relation`，原始 relationName 不保留；下次正向生成时由 `createRelationName()` 重新计算 |
| Lookup 的 `@relation(onDelete: ...)` 级联删除策略 | ❌ 完全不支持 | 正反双向均不支持：反向导入被整体过滤，正向生成 `lookup.json` 中无该属性定义，`createObjectField` 调用时 onDelete 参数为 `undefined` |
| Lookup 的 `@relation(onUpdate: ...)` 级联更新策略 | ❌ 完全不支持 | 正反双向均不支持：同上，onUpdate 参数为 `undefined` |
| Lookup 的 `@relation(map: "...")` 关系列映射 | ❌ 完全不支持 | 正反双向均不支持：同上，map 参数为 `undefined` |
