...
**关键行为**：`parseInt(value, 10)` 遵循三阶段流程（这是误判的根本原因）：
1. **先跳过所有前导空白字符**（空格 ` `、Tab `\t`、换行 `\n`、回车 `\r`、换页 `\f`、垂直制表 `\v`、以及 Unicode 空白字符等），直到遇到第一个非空白字符；
2. **从第一个非空白字符开始解析数字前缀**（`+`/`-` 符号可选，后续必须是十进制数字 0-9）；
3. **遇到任何非数字字符（包括中间空格、小数点 `.`、`e`/`E`、字母、连字符、下划线等）立即停止**，并返回已解析到的数字。
校验通过时**返回原始完整字符串 value**（而非 parseInt 解析出的数字），这意味着空格、字母、UUID 分隔符等后缀会被原样透传给 Segment。
...
| 13 | `"123 456"` | `123` | ✅ | `"123 456"` | ⚠️ 数字 + **中间空格** + 数字（parseInt 阶段 3：遇中间空格停止） |
| 14 | `" 123"` | `123` | ✅ | `" 123"` | ⚠️ **前导空格** + 数字（parseInt 阶段 1：先跳过前置空白，阶段 2 再解析数字前缀） |
| 15 | `"\t123"` | `123` | ✅ | `"\t123"` | ⚠️ **前导 Tab** + 数字（parseInt 阶段 1：跳过前置空白） |
| 16 | `"\n123"` | `123` | ✅ | `"\n123"` | ⚠️ **前导换行** + 数字（parseInt 阶段 1：跳过前置空白） |
